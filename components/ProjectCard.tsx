...
export default function ProjectCard({ project, featured = false, compact = false, diminished = false }: { project: Project, featured?: boolean, compact?: boolean, diminished?: boolean }) {
  return (
    <Link href={project.href} className={`block group h-full ${diminished ? 'scale-[0.90] hover:scale-[0.93] transition-all duration-500' : ''}`}>
      <div className="bg-white/[0.01] border border-white/[0.05] rounded-xl overflow-hidden transition-all duration-200 hover:border-white/[0.15] hover:-translate-y-[1px] hover:shadow-[0_4px_20px_rgb(0,0,0,0.3)] h-full">
        {!compact && (
          <div className={`relative bg-black w-full overflow-hidden border-b border-white/[0.05] ${featured ? 'aspect-[4/3] sm:aspect-[16/10]' : 'aspect-[4/3]'}`}>
            <Image
              src={project.image}
              alt={project.title}
              fill
        ...
        <div className={`p-4 ${featured ? 'md:p-6 md:pb-5' : 'pb-3.5'}`}>
